...
import {parsePostContentResponse} from "../utils/parser.post.content.response";
...
export class GptService implements IGptService {
    ...
    async jsonRequest(promptMessage: any): Promise<any> {
        try {
            ...
            if (requestType === "generate_post") {
                json = parsePostContentResponse(rawString)
            } else {
                try {
                    const jsonString = rawString.replace(/```json\n|```/g, '');
